...
# Bind Caliper to Besu (only needs to be done once)
bind_caliper() {
...
run_single_benchmark() {
  ...
  while [ $attempt -le $RETRY_ATTEMPTS ] && [ "$success" = false ]; do
    ...
    if [ "$NODE_TYPE" = "geth" ]; then
      network_config="networks/ethereum/geth-network.json"
    fi
