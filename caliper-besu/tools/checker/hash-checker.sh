...
# Enhanced Contract Hash Checker
...
main() {
    echo "=================================================="
    echo "    Contract Hash Checker v2.0"
    echo "=================================================="
    ...
    if [ "$runtime_stripped_hash" = "$deployed_stripped_hash" ]; then
        ...
        log_success "Runtime bytecode matches deployed bytecode (after CBOR stripping)"
        echo "✅ Contract verification: PASSED"
        exit 0
    elif [ "$runtime_hash" = "$deployed_hash" ]; then
        log_success "🎉 VERIFICATION SUCCESSFUL!"
        log_success "Runtime bytecode matches deployed bytecode exactly"
        echo "✅ Contract verification: PASSED"
        exit 0
    elif [ "$runtime_hash" = "$deployed_stripped_hash" ]; then
        log_success "🎉 VERIFICATION SUCCESSFUL!"
        log_success "Runtime bytecode matches deployed bytecode (deployed had CBOR metadata)"
        echo "✅ Contract verification: PASSED"
        exit 0
    else
        log_error "❌ VERIFICATION FAILED!"
        log_error "Runtime bytecode does not match deployed bytecode"
